<?php
        ...
        if(!isset($chck))
        {
            echo "<script>alert('Tidak Ada Investasi yang di Pilih'); window.location='media.php?module=penghapusanc';</script>";
        }else {
    ?>
...
<div class="page-content">
    ...
    <!-- form  -->
    <form action="modul/mod_penghapusanc/aksi_hapus.php" method="post" role="form">
        <div class="block">
        <h6 class="heading-hr"><i class="icon-checkmark-circle"></i> Form Penghapusan</h6>
            <div class="row">
                ...
                <?php
                    foreach ($chck as $key) { 
                        $sql = mysql_query("SELECT * FROM invesc, barang, subdinas where invesc.skpd=subdinas.kode_subdinas and barang.kode_barang=invesc.kode_barang and subdinas.kode_subdinas = '$_SESSION[dinas]' AND id_invesc='$key' order by invesc.id_invesc  DESC, invesc.register DESC ");
                        while($r=mysql_fetch_array($sql)){

                    ?>
                        <input type="hidden" name="id[]" value="<?= $r['id_invesc']?>" class="form-control">
