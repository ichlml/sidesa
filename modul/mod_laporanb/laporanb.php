...
$aksi="modul/mod_laporanb/aksi_laporanb.php";
      ...
      <div class="panel-body">
        <form method="get" role="form">
            <!-- <input type="hidden" name="act" value="cari"  /> -->
                      <input type="hidden" name="module" value="laporanB"  />
              <div class="block">
                  <div class="row">
                          <div class="col-md-6">
...
?>
		...
		<!-- main content -->
    <center>	<h3>KARTU INVENTARIS BARANG MILIK DESA<br/><small>MESIN DAN PERALATAN </small></h3></center>
    <?php
        if ($_SESSION['level']=='admin' OR $_SESSION[upt]){
...
<?php } ?>
    ...
    <td rowspan="2" width="20%">Keterangan</td>
  </tr>
  <tr>
    <td>Tanggal</td>
    <td>Banyaknya</td>
...
if ($_SESSION['level']=='admin' OR $_SESSION[upt]){ 
	...
	$tampil = mysql_query("Select  skpd, kode_barang, nama_barang, nama_subdinas,merk, bahan, ukuran, sumber_dana, tanggal_pembelian,  sum(jumlah) as jumlah, harga, sum(total) as total, keterangan from  (
    SELECT a.id_invesb, a.skpd, a.kode_barang, b.nama_barang, c.nama_subdinas,a.merk, a.bahan, a.ukuran, a.sumber_dana, a.tanggal_pembelian,  a.jumlah,( a.harga+ ifnull(d.harga,0)) as harga, (a.jumlah* (a.harga+ifnull(d.harga,0))) as total,  a.keterangan FROM invesb a 
    Inner join barang b on a.kode_barang=b.kode_barang
    Left join (select id_inves, kode_barang, sum(nilai) as harga from tb_pemeliharaan where tgl_documen<='$_SESSION[tgl1]' group by id_inves, kode_barang) d on a.kode_barang=d.kode_barang and a.id_invesb=d.id_inves
    ...
    where a.skpd like '$skpd' and a.tanggal_pembelian <= '$_SESSION[tgl1]' and e.kode_barang is null and e.id_inves is null)  as x
      group by skpd, kode_barang, nama_barang, nama_subdinas,merk, bahan, ukuran, sumber_dana, tanggal_pembelian, harga, keterangan
      order by kode_barang DESC LIMIT $posisi,$batas") or die (mysql_error());
      $jmldata=mysql_num_rows(mysql_query("SELECT a.id_invesb, a.skpd, a.kode_barang, b.nama_barang, c.nama_subdinas,a.merk, a.bahan, a.ukuran, a.sumber_dana, a.tanggal_pembelian,  a.jumlah,( a.harga+ ifnull(d.harga,0)) as harga, (a.jumlah* (a.harga+ifnull(d.harga,0))) as total,  a.keterangan FROM invesb a 
      Inner join barang b on a.kode_barang=b.kode_barang
      Left join (select id_inves, kode_barang, sum(nilai) as harga from tb_pemeliharaan where tgl_documen<='$_SESSION[tgl1]' group by id_inves, kode_barang) d on a.kode_barang=d.kode_barang and a.id_invesb=d.id_inves
      left join (select id_inves, kode_barang from penghapusan where tgl_sk<='$_SESSION[tgl1]') e on  a.kode_barang=e.kode_barang and a.id_invesb=e.id_inves
      Inner join subdinas c on a.skpd=c.kode_subdinas
      where a.skpd like '$skpd' and a.tanggal_pembelian <= '$_SESSION[tgl1]' and e.kode_barang is null and e.id_inves is null")) or die (mysql_error());
} else {
    ...
    $tampil = mysql_query("Select  skpd, kode_barang, nama_barang, nama_subdinas,merk, bahan, ukuran, sumber_dana, tanggal_pembelian,  sum(jumlah) as jumlah, harga, sum(total) as total, keterangan from  (
      ...
      where a.skpd = '$_SESSION[dinas]' and a.tanggal_pembelian <= '$_SESSION[tgl1]' and e.kode_barang is null and e.id_inves is null)  as x
        ...
        left join (select id_inves, kode_barang from penghapusan where tgl_sk<='$_SESSION[tgl1]') e on  a.kode_barang=e.kode_barang and a.id_invesb=e.id_inves
        Inner join subdinas c on a.skpd=c.kode_subdinas
        where a.skpd = '$_SESSION[dinas]' and a.tanggal_pembelian <= '$_SESSION[tgl1]' and e.kode_barang is null and e.id_inves is null")) or die (mysql_error());
  }
...
<?php } ?>
    ...
    <td align="left">&nbsp;<?php echo"$r[ukuran]"; ?></td>
...
<!--	              <td align="right" bgcolor="#CCCCCC"><?php
 ...
 // $aaaa=mysql_query("select sum(harga) as jumlahtotal from invesb");
									 // } else {
								 // $aaaa=mysql_query("select sum(harga) as jumlahtotal from invesb where skpd = '$_SESSION[dinas]' ");
	 // }
...
// $d=mysql_fetch_array($aaaa); 
		    ...
		    // $jmldata=mysql_num_rows(mysql_query("SELECT * FROM invesa $dinnassa "));
...
?>            <div class="form-actions text-right">
				 ...
				 if ($_SESSION['level']=='admin' OR $_SESSION[upt]){ 
?>
<a href="modul/mod_laporanb/excel.php"><input type="submit" value="Eksport to Excel" class="btn btn-primary"></a> 
<?php } else { ?>
  <a href="modul/mod_laporanb/excel.php"><input type="submit" value="Eksport to Excel" class="btn btn-primary"></a> 
<?php } ?>
