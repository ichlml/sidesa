<?php
        ...
        if(!isset($chck))
        {
            echo "<script>alert('Tidak Ada Investasi yang di Pilih'); window.location='media.php?module=penghapusane';</script>";
        }else {
    ?>
...
<div class="page-content">
    ...
    <!-- form  -->
    <form action="modul/mod_penghapusane/aksi_hapus.php" method="post" role="form">
        <div class="block">
        <h6 class="heading-hr"><i class="icon-checkmark-circle"></i> Form Penghapusan</h6>
            <div class="row">
                ...
                <?php
                    foreach ($chck as $key) { 
                        $sql = mysql_query("SELECT * FROM invese, barang, subdinas where invese.skpd=subdinas.kode_subdinas and barang.kode_barang=invese.kode_barang and subdinas.kode_subdinas = '$_SESSION[dinas]' AND id_invese='$key' order by invese.id_invese  DESC, invese.register DESC ");
                        while($r=mysql_fetch_array($sql)){

                    ?>
                        <input type="hidden" name="id[]" value="<?= $r['id_invese']?>" class="form-control">
