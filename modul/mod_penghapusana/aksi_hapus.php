  ...
  if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
        ...
        if(isset($_POST['hapus']))
        {
            ...
            for ($i=0; $i < count($_POST['id']); $i++) { 
                ...
                // print_r($kode_barang);
                // exit;
                $sql = mysql_query("UPDATE invesa SET status_penggunaan = '0' where id_invesa = '$idinves' ");
                $sql_Tambah = mysql_query("INSERT INTO penghapusan values('','$idinves', '$kode_barang', '$kode_bidang', '$skpd', '$tgl', '$nosk', '$tgl_sk')");
            }
            
                echo "<script>alert('Berhasil'); window.location='../../media.php?module=penghapusana';</script>";
