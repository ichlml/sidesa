...
$aksi="modul/mod_laporane/aksi_laporane.php";
      ...
      <div class="panel-body">
        <form method="get" role="form">
            <!-- <input type="hidden" name="act" value="cari"  /> -->
                      <input type="hidden" name="module" value="laporanE"  />
              <div class="block">
                  <div class="row">
                          <div class="col-md-6">
...
?>
		...
		<!-- main content -->
    <center>	<h3>KARTU INVENTARIS BARANG MILIK DESA <br/><small>ASET TETAP LAINNYA BUKU </small></h3></center>
    <?php
        if ($_SESSION['level']=='admin' OR $_SESSION[upt]){
...
if ($_SESSION['level']=='admin' OR $_SESSION[upt]){ 
	...
	$tampil = mysql_query("Select  skpd, kode_barang, nama_barang, nama_subdinas, bahan,  sumber_dana, tanggal_pembelian,  sum(jumlah) as jumlah, harga, sum(total) as total, keterangan from  (
    SELECT a.id_invese, a.skpd, a.kode_barang, b.nama_barang, c.nama_subdinas, a.bahan, a.sumber_dana, a.tanggal_pembelian,  a.jumlah,( a.harga+ ifnull(d.harga,0)) as harga, (a.jumlah* (a.harga+ifnull(d.harga,0))) as total,  a.keterangan FROM invese a 
    Inner join barang b on a.kode_barang=b.kode_barang
    Left join (select id_inves, kode_barang, sum(nilai) as harga from tb_pemeliharaan where tgl_documen<='$_SESSION[tgl1]' group by id_inves, kode_barang) d on a.kode_barang=d.kode_barang and a.id_invese=d.id_inves
    ...
    where a.skpd like '$skpd' and a.tanggal_pembelian <= '$_SESSION[tgl1]' and e.kode_barang is null and e.id_inves is null)  as x
      group by skpd, kode_barang, nama_barang, nama_subdinas, bahan, sumber_dana, tanggal_pembelian, harga, keterangan
      order by kode_barang
     DESC LIMIT $posisi,$batas") or die (mysql_error());
     $jmldata=mysql_num_rows(mysql_query(" SELECT a.id_invese, a.skpd, a.kode_barang, b.nama_barang, c.nama_subdinas, a.bahan, a.sumber_dana, a.tanggal_pembelian,  a.jumlah,( a.harga+ ifnull(d.harga,0)) as harga, (a.jumlah* (a.harga+ifnull(d.harga,0))) as total,  a.keterangan FROM invese a 
     Inner join barang b on a.kode_barang=b.kode_barang
     Left join (select id_inves, kode_barang, sum(nilai) as harga from tb_pemeliharaan where tgl_documen<='$_SESSION[tgl1]' group by id_inves, kode_barang) d on a.kode_barang=d.kode_barang and a.id_invese=d.id_inves
     left join (select id_inves, kode_barang from penghapusan where tgl_sk<='$_SESSION[tgl1]') e on  a.kode_barang=e.kode_barang and a.id_invese=e.id_inves
     Inner join subdinas c on a.skpd=c.kode_subdinas
...
<?php } ?>
    ...
    <td align="left">&nbsp;<?php //echo"$r[merk] $type"; ?></td>
...
<!--	              <td align="right" bgcolor="#CCCCCC"><?php
 ...
 // $aaaa=mysql_query("select sum(harga) as jumlahtotal from invese");
									 // } else {
								 // $aaaa=mysql_query("select sum(harga) as jumlahtotal from invese where skpd = '$_SESSION[dinas]' ");
	 // }
...
// $d=mysql_fetch_array($aaaa); 
		    ...
		    // $jmldata=mysql_num_rows(mysql_query("SELECT * FROM invese $dinnassa "));
...
?>            <div class="form-actions text-right">
				 ...
				 if ($_SESSION['level']=='admin' OR $_SESSION[upt]){ 
?>
                     	<a href="modul/mod_laporane/excel.php"><input type="submit" value="Eksport to Excel" class="btn btn-primary"></a> 
<?php } else { ?>
